...
public class RegistroHospede extends JFrame {
	...
	//pega a reserva da view anterior
	private Reserva reserva = ReservasView.reserva;
	...
	public RegistroHospede() {
		...
		txtDataN.getCalendarButton().setIcon(new ImageIcon(ReservasView.class.getResource("/imagenes/icon-reservas.png")));
		txtDataN.getCalendarButton().setFont(new Font("Roboto", Font.PLAIN, 12));
		txtDataN.setBounds(560, 278, 285, 36);
		txtDataN.getCalendarButton().setBounds(560, 278, 285, 36);
		...
		JLabel lblNumeroReserva = new JLabel("CÓDIGO DA RESERVA");
	...
	private void preecherNumemroReserva() {

		String idReseva = this.reserva.getCodigo();
		txtNreserva.setText(idReseva);
	}
	...
	private Boolean salvar() {
		if(txtDataN.getDate() != null && !txtNome.getText().isEmpty() && !txtSobrenome.getText().isEmpty() && !txtTelefone.getText().isEmpty() ) {
			Hospede hospede = new Hospede(
					txtNome.getText(),
					txtSobrenome.getText(),
					converterData(txtDataN),
					txtNacionalidade.getSelectedItem().toString(),
					txtTelefone.getText(),
					txtNreserva.getText()
				
			);
			reservaController.salvar(this.reserva);
	...
	private LocalDate converterData(JDateChooser data) {
		return data.getDate().toInstant().atZone(ZoneId.systemDefault()).toLocalDate().plusDays(1);
	}
