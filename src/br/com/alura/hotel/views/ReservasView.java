...
public class ReservasView extends JFrame {
	...
	public ReservasView() {
		...
		txtFormaPagamento.setBackground(Color.white);
		...
		txtFormaPagamento.setModel(new DefaultComboBoxModel(new String[] { "CARTAO DE CREDITO", "CARTAO DE DEBITO",
				"DINHEIRO"}));
	...
	private BigDecimal preencherValorReserva() {
		BigDecimal valorDiaria = new BigDecimal(200);
		LocalDate dataEntrada = converterData(txtDataE);
		
		LocalDate dataSaida = converterData(txtDataS);
		long dias = ChronoUnit.DAYS.between(dataEntrada, dataSaida);
		BigDecimal valorTotal = valorDiaria.multiply(new BigDecimal(dias));
		txtValor.setText(valorTotal.toString());
	...
	private LocalDate converterData(JDateChooser data) {
		return data.getDate().toInstant().atZone(ZoneId.systemDefault()).toLocalDate();
	}
}
